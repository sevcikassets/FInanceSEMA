...
@requires_db
def test_snapshot_daily_alerts_persists_today_and_upserts_on_rerun(db_session, portfolio_id):
    """Mirrors the original Excel workbook's daily log of watchlist-limit
    and portfolio-drawdown alerts - captured once per day so they can be
    looked up retroactively. Rerunning the same day must replace, not
    duplicate, that day's rows (e.g. AAPL no longer breaching its limit)."""
    from app import stock_services
    from app.models import DailyAlertLog, PortfolioPosition, WatchlistStock

    watch = WatchlistStock(
        portfolio_id=portfolio_id, watched_on=date(2024, 1, 1), name="Apple", ticker="AAPL",
        limit_price=Decimal("200"), current_price=Decimal("180"), currency="USD",
    )
    db_session.add(watch)
    db_session.add(
        PortfolioPosition(
            portfolio_id=portfolio_id, ticker="MSFT", name="Microsoft", quantity=Decimal("1"),
            current_price=Decimal("100"), currency="USD", market_value_czk=Decimal("2000"),
            invested_czk=Decimal("3000"), profit_czk=Decimal("-1000"), profit_pct=Decimal("-0.3333"),
        )
    )
    db_session.commit()

    stock_services.snapshot_daily_alerts(db_session, portfolio_id, threshold_pct=Decimal("10"))
    db_session.commit()

    today = date.today()
    rows = db_session.scalars(
        select(DailyAlertLog).where(DailyAlertLog.portfolio_id == portfolio_id, DailyAlertLog.stat_date == today)
    ).all()
    by_type_ticker = {(row.alert_type, row.ticker): row for row in rows}
    assert set(by_type_ticker) == {("watchlist_limit", "AAPL"), ("drawdown", "MSFT")}
    assert by_type_ticker[("watchlist_limit", "AAPL")].detail["limit_price"] == 200.0
    assert by_type_ticker[("drawdown", "MSFT")].detail["profit_czk"] == -1000.0

    # AAPL no longer breaches its limit - the next snapshot must drop it,
    # not leave a stale row alongside a fresh duplicate.
    watch.current_price = Decimal("250")
    db_session.commit()
    stock_services.snapshot_daily_alerts(db_session, portfolio_id, threshold_pct=Decimal("10"))
    db_session.commit()
    ...
    assert {(row.alert_type, row.ticker) for row in rows_after} == {("drawdown", "MSFT")}


@requires_db
def test_stock_alert_history_endpoint_scopes_by_portfolio_and_date(client, db_session, portfolio_id):
    from app.models import DailyAlertLog, Portfolio

    other_portfolio = Portfolio(name="Jiny Subjekt Alerts")
    ...
    db_session.add_all(
        [
            DailyAlertLog(
                portfolio_id=portfolio_id, stat_date=date(2024, 6, 1), alert_type="watchlist_limit", ticker="AAPL",
                name="Apple", detail={"current_price": 180.0, "limit_price": 200.0, "currency": "USD"},
            ),
            DailyAlertLog(
                portfolio_id=portfolio_id, stat_date=date(2024, 1, 1), alert_type="drawdown", ticker="MSFT",
                name="Microsoft", detail={"profit_pct": -0.33, "profit_czk": -1000.0, "market_value_czk": 2000.0},
            ),
            DailyAlertLog(
                portfolio_id=other_portfolio.id, stat_date=date(2024, 6, 1), alert_type="drawdown", ticker="GOOG",
                name="Google", detail={"profit_pct": -0.5, "profit_czk": -5000.0, "market_value_czk": 5000.0},
            ),
        ]
    )
    db_session.commit()
    ...
    full = client.get("/stocks/alerts/history", headers=headers, params={"portfolio_id": str(portfolio_id)})
    assert full.status_code == 200
    tickers = {row["ticker"] for row in full.json()}
    assert tickers == {"AAPL", "MSFT"}  # scoped to this portfolio, not the other one's GOOG row

    filtered = client.get(
        "/stocks/alerts/history",
        headers=headers,
        params={"portfolio_id": str(portfolio_id), "date_from": "2024-05-01"},
    )
    assert filtered.status_code == 200
    assert [row["ticker"] for row in filtered.json()] == ["AAPL"]


@requires_db
def test_refresh_prices_endpoint_writes_alert_history_snapshot(client, db_session, portfolio_id, monkeypatch):
    """End-to-end wiring check: POST /stocks/refresh-prices (the "Aktualizovat
    ceny" action) must itself trigger today's alert-history snapshot, not
    just the live /stocks/alerts computation."""
    from app import stock_services
    from app.models import DailyAlertLog, WatchlistStock

    db_session.add(
        WatchlistStock(
            portfolio_id=portfolio_id, watched_on=date(2024, 1, 1), name="Apple", ticker="AAPL",
            limit_price=Decimal("200"), currency="USD",
        )
    )
    db_session.commit()
    ...
    login = client.post("/auth/login", json={"username": "admin", "password": "finance"})
    headers = {"Authorization": f"Bearer {login.json()['token']}"}
    response = client.post("/stocks/refresh-prices", headers=headers, params={"portfolio_id": str(portfolio_id)})
    assert response.status_code == 200

    today = date.today()
    logged = db_session.scalars(
        select(DailyAlertLog).where(
            DailyAlertLog.portfolio_id == portfolio_id, DailyAlertLog.stat_date == today, DailyAlertLog.ticker == "AAPL"
        )
    ).all()
    assert len(logged) == 1
    assert logged[0].alert_type == "watchlist_limit"
